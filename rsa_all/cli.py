...
import rsa_all
import rsa_all.bigfile
import rsa_all.pkcs1

HASH_METHODS = sorted(rsa_all.pkcs1.HASH_METHODS.keys())
...
def keygen():
    ...
    (pub_key, priv_key) = rsa_all.newkeys(keysize)
...
class EncryptOperation(CryptoOperation):
    ...
    def perform_operation(self, indata, pub_key, cli_args=None):
        ...
        return rsa_all.encrypt(indata, pub_key)
...
class DecryptOperation(CryptoOperation):
    ...
    operation_past = 'decrypted'
    operation_progressive = 'decrypting'
    key_class = rsa_all.PrivateKey

    def perform_operation(self, indata, priv_key, cli_args=None):
        '''Decrypts files.'''

        return rsa_all.decrypt(indata, priv_key)
...
class SignOperation(CryptoOperation):
    ...
    def perform_operation(self, indata, priv_key, cli_args):
        ...
        return rsa_all.sign(indata, priv_key, hash_method)
...
class VerifyOperation(CryptoOperation):
    ...
    def perform_operation(self, indata, pub_key, cli_args):
        ...
        try:
            rsa_all.verify(indata, signature, pub_key)
        except rsa_all.VerificationError:
...
class EncryptBigfileOperation(BigfileOperation):
    ...
    def perform_operation(self, infile, outfile, pub_key, cli_args=None):
        ...
        return rsa_all.bigfile.encrypt_bigfile(infile, outfile, pub_key)
...
class DecryptBigfileOperation(BigfileOperation):
    ...
    def perform_operation(self, infile, outfile, priv_key, cli_args=None):
        ...
        return rsa_all.bigfile.decrypt_bigfile(infile, outfile, priv_key)
